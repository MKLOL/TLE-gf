...
Grok is a provider route over xAI's Responses API: `;llm +grok <question>` and a channel message beginning with the literal text `@grok <question>` share the same reply/history/image pipeline. Its system prompt keeps Gemini's broad subject scope and truthfulness rules, adds competitive-programming problem solving, and uses an aggressive roast-comedy voice with direct banter and uncensored profanity. Ambiguous Grok questions route through xAI, so Grok never depends on Gemini quota or credentials. `XAI_MODELS` is a strongest-to-weakest fallback ladder and defaults to the singular `XAI_MODEL` (`grok-4.5`). Model-not-found/access failures skip the unavailable model rather than trying it against every key.
