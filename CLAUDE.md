...
### `;virtual` — blind random contest for gitgud points

`;virtual` picks a finished, standard contest for the user's division (same split as `;vc`) that they have never touched, and asks them to confirm **without showing it**. Confirming (a button, invoker-only, ten-minute offer) reveals the contest and opens a `virtual_session` (migration 1.62.0): the user has 30 minutes to start the virtual on Codeforces, and solves count until start grace plus the contest's duration. There is deliberately no cancel after reveal — cancelling would be a free reroll.

The bot cannot start a Codeforces virtual; it verifies afterwards from `user.status`. A solve counts only if `author.participantType == 'VIRTUAL'`, the party is one member, `author.startTimeSeconds` (the virtual's own start) is after the confirmation and before expiry, and `relativeTimeSeconds` is within the contest length. `;virtual claim` credits new solves — repeatable, and `;virtual` finalises an expired session before offering a new one so late claims are never lost. Each credited problem is written as a completed challenge (`credit_virtual_solve`, one transaction) with the usual rating-delta score against the gitgud base rating, so `;gitlog`, `;gitgudders` and the monthly board need no special casing. The user's live gitgud problem is skipped (`;gotgud` pays that one), unrated problems are reported and skipped, and a `credited` list on the session makes repeated claims idempotent.

### Complaint tags and deferred notifications

`;complain tag <id> <tag>` attaches any word (`normalize_tag`: 1–32 of `a-z0-9_-`, lower-cased; `all`/`open`/`resolved`/`untagged` reserved). Tagged complaints leave the default views: `;complain list` and `manage` hide them, `list all` shows everything, `list <tag>` shows one tag. The API does not hide them: every payload carries `tags`, `?tag=untagged` gives the Discord view and `?tag=<name>` one tag. Table `complaint_tag`, migration 1.61.0.

Resolutions made through the HTTP API do **not** notify immediately: `resolve_complaint(defer_notification=True)` parks the row as `queued`, which the retry worker never touches. `ComplaintService.__init__` captures `git rev-parse HEAD` at cog load (`tle/util/git_state.py`; a later `git pull` moves HEAD but not the loaded code, so live HEAD is never consulted and the capture is eager, not on first use) and `_deployed()` asks `git merge-base --is-ancestor <sha> <that head>`. That check runs at every start over everything queued (`release_deployed`) and at resolve time, so a fix the bot already runs is released immediately rather than waiting for one more restart. `release_complaint_notification(id, commit_url)` requires the row to still be queued *and* still carry the verified commit, closing the reopen-and-re-resolve race. `_notify` refuses to send from `queued`, so repeating the resolve from the API or from Discord cannot leak it early; reopening discards it. Without a git checkout (the Dockerfile drops `.git/`) nothing is ever released and one WARNING per start says so. `;complain resolve` in Discord still notifies immediately. The API lists tagged complaints by default (`tags` on every row; `tag=untagged` for the Discord view), unlike `;complain list`. Complaints filed from Discord also capture the five preceding messages (`_complaint_context.py`, migration 1.60.0), embed-aware and secret-redacted, served on the API detail route as `context`.

### Problemset cache self-heal

`get_visited_contests` (used by `;vc` and `;virtual`) always counts a submission's own contest as visited; the `problem_to_contests` map only adds the sibling Div. 1/Div. 2 round. That map is built from the disk problemset cache (`problem2`), which is filled per contest from `contest.standings` and, by design, retried only for two weeks after a contest ends — so a fetch that failed inside that window left the contest with no problems for good, and every submission in it mapped to nothing (complaint #299, Spectral Cup 2222). `ProblemsetCache._backfill_missing` now fetches up to 20 such contests per hourly tick, newest first, remembering unfetchable ids for the rest of the process; `;cache problemsets missing` (admin) does the whole backlog at once. The first tick that finds a gap logs one WARNING with the count.

### `;queens time` — solve time from a screen recording

Reply to a message carrying a screen recording of a Queens solve (or attach one to the command) and the bot answers with the time to hundredths, e.g. `0:44.75 (±0.04 s)`. LinkedIn only shows `m:ss`, but every flip of the seconds digit samples its internal clock, so the analysis (`tle/util/queens_video_*.py`, ported from the standalone `Queens/tools/queens_time_from_video.py`) locates the timer by its ~1 s change cadence, anchors the absolute count via the tens-digit roll and font templates (`tle/util/queens_timer_digits.npz`), finds the finishing move (last board change before Reset greys out), and reports `seconds shown + (t_finish − t_last_flip)`. It shells out to `ffmpeg`/`ffprobe` (in the Dockerfile; not a Python dependency) and streams the downscaled video, but the full-resolution timer strip of every frame is held in RAM at once, so peak memory scales with frame count rather than file size (~280 MB for 7.7k frames, ~1 GB for 19k frames at 1080p/60 fps). `_QUEENS_VIDEO_MAX_FRAMES` (10k) is therefore checked via `probe_frame_count` before any decode and is the knob to lower on a small host. The cog side (`_mgimpl_queenstime.py`) downloads the attachment to a scratch directory, runs the analysis in a worker thread behind a one-at-a-time gate, and passes a monotonic deadline into the decoder so a timeout actually kills ffmpeg rather than orphaning it. Timer location is two-tier: the primary search wants one 4x4 analysis block to carry the whole ~1 s tick chain, which holds for 16 px digits (880-wide recordings). On lower-resolution phones (digits of 9 to 13 px) successive transitions land in different blocks and a 3-second solve's timer loses the tie to the board's click chains, so when the primary pass fails to find a usable chain `analyze` retries with `find_timer_blocks(dilate=True)`: chains are searched over each block's 3x3 neighbourhood, with a tighter 0.2 s tolerance, chains that repaint a whole neighbourhood or change more than 2% of the frame at an event are dropped (board clicks, result-screen confetti), and the rest are scored by regularity and by how quiet the frame was at the ticks (a tick alone changes ~5 blocks). A third tier handles 2-second solves, which have exactly two ticks because "0:00" appears inside the board's own scene transition: two-event chains are too weak to rank (the status-bar clock also ticks), so `find_timer_blocks(ranked=True)` returns the best few clusters and `_analyze` accepts the first whose digit OCR reads the same count as the tick chain; without OCR agreement it fails rather than guess. Neither fallback runs on clips the primary pass handles, so the 130-clip baseline is unchanged by them. The command deliberately does not record anything: it is a measurement, not a result submission. The port was verified identical on all 130 reference recordings; `python -m tle.util.queens_video_analyze VIDEO...` reproduces that CLI check. Tests stub numpy, so the algorithm itself is not unit-tested here; only the command wiring is.
...
| `tle/cogs/complain.py`, `_complaint_manage.py`, `_complaint_context.py` | Complaint commands, remove-button view, preceding-message capture |
| `tle/util/complaints.py`, `complaint_http.py`, `git_state.py` | Complaint service (deferred API notifications), HTTP API, deploy check |
| `tle/cogs/_codeforces_virtual.py`, `tle/util/db/virtual_db.py` | `;virtual` blind random contest and its session/credit storage |
...
| `tle/cogs/_mgimpl_queenstime.py` | `;queens time`: attachment pickup, scratch download, gated analysis, result embed |
| `tle/util/queens_video_decode.py`, `queens_video_timer.py`, `queens_video_analyze.py` | ffmpeg streaming decode, timer location/OCR, and the solve-time estimator |
