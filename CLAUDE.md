...
**Two-stage pipeline** (`_llm_pipeline.py`), adapted from [MKLOL/TLE-gf#10](https://github.com/MKLOL/TLE-gf/pull/10): a cheap routing call classifies the question `direct` / `requires_context` / `requires_reply_chain`, and only then is channel history collected (`_llm_history.py`). Routing goes to `LLM_MODELS[0]` (the ladder is cheapest-first) with structured output forcing a valid label, and any failure falls back to `requires_context` — losing the optimisation must never block the answer, and answering an ambiguous question with history beats telling the user the history is unavailable. It costs a second request per question; `LLM_CONTEXT_ENABLED=0` turns it off. **A reply always gathers its surrounding window regardless of the router's verdict** — reading history costs a Discord call, not an API one.

Because context is now the *default* rather than the exception, two prompt shapes have to stay honest about what was actually gathered. `build_context_prompt` only claims a "marked replied-to message" when there really is one — the marker exists solely for replies, and pointing at it on a plain context question sends the model looking for something the transcript does not contain. `build_question_prompt(context_requested=True)` is the other side: the router asked for history and `gather` came back empty (quiet channel, aged-out window, lost Read Message History), so the prompt says so instead of silently answering a question it was told might depend on messages it cannot see.

**Transcripts are embed-aware** (`llm_history.message_text`). The cog answers in embeds, so `message.content` is empty for its own output — quoting a reply to a `;llm` answer produced `(empty message)` until embed title/description/fields/footer were rendered into the transcript. For the same reason `_is_usable` takes `include_bot`: the bot is filtered out of ordinary history, but the *direct reply target* is context even when the bot wrote it.
