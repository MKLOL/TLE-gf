...
`;virtual` picks a finished, standard contest for the user's division (same split as `;vc`) that they have never touched, and asks them to confirm **without showing it**. Confirming (a button, invoker-only, ten-minute offer) reveals the contest and opens a `virtual_session` (migration 1.62.0): the user has 10 minutes to start the virtual on Codeforces, and solves count until start grace plus the contest's duration. There is deliberately no cancel after reveal — cancelling would be a free reroll.
