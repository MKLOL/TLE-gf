...
Reply to a message carrying a screen recording of a Queens solve (or attach one to the command) and the bot answers with the time to hundredths, e.g. `0:44.75 (±0.04 s)`. LinkedIn only shows `m:ss`, but every flip of the seconds digit samples its internal clock, so the analysis (`tle/util/queens_video_*.py`, ported from the standalone `Queens/tools/queens_time_from_video.py`) locates the timer by its ~1 s change cadence, anchors the absolute count via the tens-digit roll and font templates (`tle/util/queens_timer_digits.npz`), finds the finishing move (last board change before Reset greys out), and reports `seconds shown + (t_finish − t_last_flip)`. It shells out to `ffmpeg`/`ffprobe`, resolved by `queens_video_decode.find_tool`: `$FFMPEG_DIR`, then PATH, then `/usr/bin` and friends, then the `static-ffmpeg` package, which downloads a static ffmpeg 5.0 build into the bot's site-packages on first use. That last rung exists because the bot process could not see the host's `/usr/bin/ffmpeg` at all (its environment is isolated from the operator's shell; the Playwright scraper had the same problem and solved it with an in-bot `pip install`), so a pip-level dependency that `poetry install` pulls on restart is the only route that needs no host access. A system install still wins when visible. The analysis streams the downscaled video, but the full-resolution timer strip of every frame is held in RAM at once, so peak memory scales with frame count rather than file size (~280 MB for 7.7k frames, ~1 GB for 19k frames at 1080p/60 fps). `_QUEENS_VIDEO_MAX_FRAMES` (10k) is therefore checked via `probe_frame_count` before any decode and is the knob to lower on a small host. The cog side (`_mgimpl_queenstime.py`) downloads the attachment to a scratch directory, runs the analysis in a worker thread behind a one-at-a-time gate, and passes a monotonic deadline into the decoder so a timeout actually kills ffmpeg rather than orphaning it. The command deliberately does not record anything: it is a measurement, not a result submission. The port was verified identical on all 130 reference recordings; `python -m tle.util.queens_video_analyze VIDEO...` reproduces that CLI check. Tests stub numpy, so the algorithm itself is not unit-tested here; only the command wiring is.
