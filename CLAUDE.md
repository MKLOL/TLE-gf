...
Resolutions made through the HTTP API do **not** notify immediately: `resolve_complaint(defer_notification=True)` parks the row as `queued`, which the retry worker never touches. On each start `ComplaintService.release_deployed` checks every queued row's commit with `git merge-base --is-ancestor <sha> HEAD` in the bot's own checkout (`tle/util/git_state.py`, the same answer `;meta git` gives) and moves only running commits to `pending`, from where the normal path delivers once. `_notify` refuses to send from `queued`, so repeating the resolve from the API or from Discord cannot leak it early; reopening discards it. `;complain resolve` in Discord still notifies immediately. Complaints filed from Discord also capture the five preceding messages (`_complaint_context.py`, migration 1.60.0), embed-aware and secret-redacted, served on the API detail route as `context`.
